...
class ShooterController:
    ...
    interpolation_override = tunable(True)
    flywheel_speed = tunable(0.0)

    wants_to_fire = will_reset_to(False)

    allowed_flywheel_error = tunable(10.0)
    allowed_bearing_error = tunable(0.1)
    ...
    def execute(self):
        ...
        if self.wants_to_fire and abs(self.shooter.flywheel_error()) < self.allowed_flywheel_error and abs(self.turret.get_error()) < allowed_bearing_error:
